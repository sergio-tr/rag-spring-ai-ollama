...
- **CI (PRs to `dev` / `main` / `master` via [`ci.yml`](../../.github/workflows/ci.yml) → [`reusable-ci-core.yml`](../../.github/workflows/reusable-ci-core.yml)):** **`core_webapp`** runs **`npm run test:coverage`**. **`core_webapp_e2e`** runs **`npm run test:e2e:ci-fast`** (`@smoke`, `retries=0`, `max-failures=1`, workers=1). **`playwright_api_smoke`** runs **`npm run test:api`**. **`integration`** + **`integration_classifier_required`** run **`pytest tests/integration`**. **`e2e_fullstack`** runs stack curl → **`test:e2e:preflight:only`** → **`test:e2e:fullstack:critical`** (6 tests: Chat/Lab/Classifier/Documents), not all 42 `@fullstack` tags — fast-fail before long retry burn. **`docker_build_smoke`** is limited to **`main`/`master`** PRs and pushes (not **`dev`** PRs).
