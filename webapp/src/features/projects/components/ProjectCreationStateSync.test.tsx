...
function projectsListKey(page: number, size: number) {
  return ["projects", getClientSessionEpoch(), page, size] as const;
}
...
describe("ProjectCreationStateSync", () => {
  ...
  it("prepends created project to list cache before refetch completes", async () => {
    ...
    const listKey = projectsListKey(0, 24);
    qc.setQueryData(listKey, { items: [summary("old", "Old")], total: 1 });
    ...
    const cached = qc.getQueryData<{ items: { id: string }[] }>(listKey);
    expect(cached?.items.map((p) => p.id)).toEqual(["new", "old"]);
    expect(useAppStore.getState().activeProject).toEqual({ id: "new", name: "New" });
  });

  it("does not throw when a project detail cache entry exists alongside the list", async () => {
    const { wrapper, qc } = createWrapper();
    const listKey = projectsListKey(0, 24);
    const detailKey = ["projects", getClientSessionEpoch(), "detail", "existing"] as const;
    qc.setQueryData(listKey, { items: [summary("existing", "Existing")], total: 1 });
    qc.setQueryData(detailKey, summary("existing", "Existing"));

    apiFetch
      .mockResolvedValueOnce({ items: [summary("existing", "Existing")], total: 1 })
      .mockResolvedValueOnce(summary("new", "New"))
      .mockResolvedValueOnce({ activeProjectId: "new" })
      .mockResolvedValueOnce({
        items: [summary("new", "New"), summary("existing", "Existing")],
        total: 2,
      });
    ...
    expect(qc.getQueryData<{ items: { id: string }[] }>(listKey)?.items.map((p) => p.id)).toEqual([
      "new",
      "existing",
    ]);
  });
  ...
  it("keeps cached project visible when refetch fails after create", async () => {
    const { wrapper, qc } = createWrapper();
    const listKey = projectsListKey(0, 24);
    qc.setQueryData(listKey, { items: [], total: 0 });
