...
Use **`PATCH {product}/conversations/{id}`** from the same UI for `presetId` / `clearPreset` and `documentFilter` (subset of project document UUIDs). Model and preset dropdowns use **`GET {product}/models`** and **`GET {product}/presets`**. Settings → General also lists **curated demo models** via **`GET {product}/model-registry`** (Ollama presence + **`POST {product}/model-registry/check`** / **`POST {product}/model-registry/pull`** for authenticated users; global DB allowlist edits stay on **`{product}/admin/models`** for admins only).
