...
**Compose layout:** `docker-compose.yml` (core + optional services behind **profiles**: `observability`, `logs`, `infra`, `ollama`, `cadvisor`, and **`rag`** for `backend-dev`). Overlays: `compose.dev.yml`, `compose.dev-webapp.yml` (`--rag`), `compose.dev-proxy.yml` (`--rag --proxy`, adds **`--profile proxy`**), `compose.obs.yml` (Spring OTLP for `backend` / `classifier-service`), `compose.gpu.yml`, `compose.ollama-remote.yml` (+ `.dev`), `compose.rag-dev-obs.yml` (`--rag --obs`), `compose.prod.yml` (+ `compose.prod-obs.yml` with `--obs`).
