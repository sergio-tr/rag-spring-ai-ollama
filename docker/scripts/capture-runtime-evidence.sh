...
# Usage: ./docker/scripts/capture-runtime-evidence.sh [--obs] [--out DIR]
set -euo pipefail
...
[ "$WITH_OBS" = true ] && [ -f "$ROOT_DIR/observability/.env" ] && COMPOSE_ARGS+=(--env-file "$ROOT_DIR/observability/.env")
