...
## Historical `GET {historical}/query`
...
**Tokens / model id:** The stale JSON **does not** include `prompt_tokens`, `completion_tokens`, or a stable `model_id` field. Optional query param `chatModel` selects the Ollama chat model for the request; the benchmark report records this param when present.
...
**Historical `GET /query`** uses the **non–project-scoped** pipeline (`configResolver.resolve(null, null, null)`); toggling project config does **not** change historical query behaviour. Scenarios that require `useRetrieval` / NER / tools / reasoning / top-k must use **`transport: product_chat`** in the scenario YAML (plus `BENCHMARK_*` env vars).
