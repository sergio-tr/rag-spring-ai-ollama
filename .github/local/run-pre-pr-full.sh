#!/usr/bin/env bash
# Complete local pre-PR gate: deterministic module checks plus strict integration and CI-like fullstack E2E.
set -euo pipefail
...
run_step "backend-clean-verify" bash -lc 'cd rag-service && ./mvnw -B clean verify'
...
run_step "e2e-fullstack-ci-fast" bash -lc '.github/local/run-e2e-fullstack-ci-like.sh'
