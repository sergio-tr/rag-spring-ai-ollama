...
class TestClassificationService:
    ...
    def test_classify_uses_default_model_id_when_not_provided(self):
        ...
        svc = ClassificationService(engine)
        svc.classify("q")
        engine.predict_detailed.assert_called_once()
        args = engine.predict_detailed.call_args
        assert args[0][0] == "q"
    ...
    def test_classify_uses_provided_model_id(self):
        ...
        engine.predict_detailed.assert_called_once()
        assert engine.predict_detailed.call_args[0][1] == "custom-model"
    ...
    def test_classify_predict_runtime_error(self):
        engine = _mock_engine(loaded=True, predict_result="COUNT_DOCUMENTS")
        engine.predict_detailed.side_effect = RuntimeError("not ready")
        svc = ClassificationService(engine)
    ...
    def test_classify_predict_other_exception(self):
        engine = _mock_engine(loaded=True, predict_result="COUNT_DOCUMENTS")
        engine.predict_detailed.side_effect = ValueError("unexpected")
        svc = ClassificationService(engine)
...
def _mock_engine(*, loaded: bool, predict_result: str, loader=None):
    ...
    engine.predict_detailed.return_value = ClassificationResult(
        query_type=predict_result,
        confidence=0.91,
        top_predictions=(TopPrediction(query_type=predict_result, confidence=0.91),),
        label_set_hash="testhash",
    )
    if loader is None:
