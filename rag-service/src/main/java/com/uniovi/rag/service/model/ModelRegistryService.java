...
/**
 * Read-only curated model registry + Ollama presence checks. Pull is delegated to {@link com.uniovi.rag.service.async.AsyncTaskService}
...
public class ModelRegistryService {
    ...
    public ModelRegistryItemDto check(String rawModelId, Boolean probeEmbeddingFlag) {
        ...
        boolean installed = ModelsCatalogService.matchesInstalledName(model.modelId(), tags.installed());
        if (!installed) {
            return new ModelRegistryItemDto(
                    model.modelId(),
                    model.modelType(),
