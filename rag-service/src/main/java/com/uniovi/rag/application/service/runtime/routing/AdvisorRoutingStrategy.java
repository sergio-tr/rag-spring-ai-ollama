...
/** Resolves advisor-first routing when full adaptive routing is off. */
@Service
public class AdvisorRoutingStrategy {

    private final AdvisorRoutingPolicy policy;
    private final RouteExecutionGateBuilder gateBuilder;

    public AdvisorRoutingStrategy(AdvisorRoutingPolicy policy, RouteExecutionGateBuilder gateBuilder) {
        this.policy = policy;
    ...
    public AdaptiveRoutingExecutionResult execute(RagConfig rag, QueryPlan plan) {
        ...
        stages.add(
                new ExecutionStageTrace(
                        ...
                        0L,
                        ExecutionStageOutcome.SUCCESS,
                        "advisor_routing=true primary=" + decision.primaryRouteKind()));
