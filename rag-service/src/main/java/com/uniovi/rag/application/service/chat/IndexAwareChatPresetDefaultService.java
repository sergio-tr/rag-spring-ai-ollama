 ...
 *
 * <p>Priority: P3 (thesis default) → compatible user/product preset → experimental fallbacks (P8, P4, P2, P0).
 */
@Service
public class IndexAwareChatPresetDefaultService {
    ...
    static Optional<UUID> resolveFromCatalog(ProjectCompatiblePresetsCatalog catalog) {
        ...
        Optional<UUID> p3 = firstSelectableProduct(catalog.productPresets(), P3_PRESET_ID);
        if (p3.isEmpty()) {
            p3 = firstSelectableExperimental(catalog.experimentalPresets(), P3_PRESET_ID);
        }
        if (p3.isPresent()) {
            return p3;
        }
        ...
        Optional<UUID> userProduct =
                catalog.productPresets().stream()
                        ...
                        .filter(item -> !P3_PRESET_ID.equals(item.preset().id()))
