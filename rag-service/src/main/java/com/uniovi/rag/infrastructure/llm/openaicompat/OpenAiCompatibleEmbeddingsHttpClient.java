...
/**
 * Low-level {@code POST /v1/embeddings} transport. Does not call {@code /v1/models}, Ollama routes, or {@code /api/**}.
 */
@Component
class OpenAiCompatibleEmbeddingsHttpClient {

    OpenAiEmbeddingResponse post(String baseUrl, String apiKey, OpenAiEmbeddingRequest body, long timeoutMs) {
        ...
        String embeddingsUrl = LlmUrlUtils.openAiEmbeddingsUrl(baseUrl);
        RestTemplate restTemplate = restTemplateFor(timeoutMs);
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.setBearerAuth(apiKey);
        HttpEntity<OpenAiEmbeddingRequest> entity = new HttpEntity<>(body, headers);
        try {
            ResponseEntity<OpenAiEmbeddingResponse> response =
                    restTemplate.exchange(embeddingsUrl, HttpMethod.POST, entity, OpenAiEmbeddingResponse.class);
            OpenAiEmbeddingResponse responseBody = response.getBody();
            ...
            throw OpenAiCompatibleChatMapper.mapHttpError(
                    e.getStatusCode().value(), e.getResponseBodyAsString(), embeddingsUrl);
