...
/**
 * Validates Ollama embedding output width against the physical {@code vector_store.embedding} column.
 */
@Service
public class EmbeddingSpaceGuard {
    ...
    public EmbeddingSpaceGuard(OllamaEmbeddingModelFactory embeddingModelFactory, RagVectorProperties ragVectorProperties) {
    ...
    public int probeOutputDimensions(String modelId) {
        ...
        if (dims <= 0) {
            throw new ResponseStatusException(
                    ...
                    "EMBEDDING_DIMENSION_UNAVAILABLE: could not determine output dimension for model '" + modelId + "'");
        }
        return dims;
    }
