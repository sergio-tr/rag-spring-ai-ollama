...
/**
 * Uses a real {@link OauthLoginService} with mocked ports so {@link OauthController} redirect Location matches
 * Google authorization URL rules (including {@code prompt}) without calling Google.
 */
...
class OauthGoogleStartRedirectWebMvcTest {
    ...
    @Test
    void startGoogle_defaultPrompt_redirectsToGoogleWithSelectAccount() throws Exception {
        mockMvc.perform(get("/api/v5/auth/oauth/google/start").param("locale", "en"))
                ...
                .andExpect(header().string(HttpHeaders.LOCATION, Matchers.containsString("prompt=select_account")))
                .andExpect(header().string(HttpHeaders.LOCATION, Matchers.containsString("state=")))
                .andExpect(header().string(HttpHeaders.LOCATION, Matchers.containsString("redirect_uri=")))
                .andExpect(header().string(HttpHeaders.LOCATION, Matchers.containsString("client_id=test-google-client-id")));
    }
}
