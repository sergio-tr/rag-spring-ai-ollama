...
@WebMvcTest(controllers = RuntimeTraceReplayBatchExportController.class)
@ContextConfiguration(classes = RagWebMvcTestApplication.class)
@AutoConfigureMockMvc(addFilters = false)
@Import({RuntimeTraceReplayBatchExportController.class, ReplayBatchRestJacksonConfiguration.class})
@ActiveProfiles("test")
class RuntimeTraceReplayBatchExportControllerTest {

    @Autowired
    private MockMvc mockMvc;

    @MockitoBean
    private RuntimeTraceReplayBatchExportService batchExportService;
    ...
    void controller_exposes_exactly_two_post_mappings() {
        long n =
                Arrays.stream(RuntimeTraceReplayBatchExportController.class.getDeclaredMethods())
    ...
    void query_string_on_trace_export_returns_400() throws Exception {
        when(batchExportService.exportByTraceIds(any(), any())).thenReturn(sampleZip("runtime-trace-replays-batch.zip"));
        mockMvc.perform(
                        post("/api/v5/runtime-traces/replays/batch/export")
                                .queryParam("x", "1")
    ...
    void query_string_on_conversation_export_returns_400() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/v5/conversations/{cid}/runtime-traces/replays/batch/export", conversationId)
                                .queryParam("y", "1")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{}"))
                .andExpect(status().isBadRequest());
    ...
    void not_attempted_returns_400() throws Exception {
        when(batchExportService.exportByTraceIds(eq(userId), any()))
                .thenThrow(new RuntimeTraceReplayBatchExportNotAttemptedException("x"));
        mockMvc.perform(
                        post("/api/v5/runtime-traces/replays/batch/export")
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void empty_trace_ids_returns_200_zip() throws Exception {
        when(batchExportService.exportByTraceIds(eq(userId), eq(List.of())))
                .thenReturn(sampleZip("runtime-trace-replays-batch.zip"));
        mockMvc.perform(
                        post("/api/v5/runtime-traces/replays/batch/export")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"traceIds\":[]}"))
                .andExpect(status().isOk())
                .andExpect(header().string("Content-Disposition", "attachment; filename=\"runtime-trace-replays-batch.zip\""));
    }
    ...
    void conversation_empty_selection_returns_200() throws Exception {
        String fn = "runtime-trace-replays-batch_conversation_" + conversationId + ".zip";
        when(batchExportService.exportByConversation(eq(userId), eq(conversationId), any(), any(), any()))
    ...
    void trace_export_never_404_after_valid_body() throws Exception {
        when(batchExportService.exportByTraceIds(eq(userId), any())).thenReturn(sampleZip("runtime-trace-replays-batch.zip"));
        mockMvc.perform(
                        post("/api/v5/runtime-traces/replays/batch/export")
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void size_overflow_returns_413() throws Exception {
        when(batchExportService.exportByTraceIds(eq(userId), any()))
                .thenThrow(new RuntimeTraceReplayBatchExportSizeExceededException("too large"));
        mockMvc.perform(
                        post("/api/v5/runtime-traces/replays/batch/export")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"traceIds\":[]}"))
    ...
    void malformed_conversation_uuid_returns_400() throws Exception {
        mockMvc.perform(
                        post("/api/v5/conversations/not-a-uuid/runtime-traces/replays/batch/export")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{}"))
                .andExpect(status().isBadRequest());
    }

    private static RuntimeTraceReplayBatchExportArtifact sampleZip(String filename) {
        byte[] content = {1, 2, 3};
        return new RuntimeTraceReplayBatchExportArtifact(
                filename, RuntimeTraceReplayBatchExportArtifact.MEDIA_TYPE_ZIP, content, content.length);
    }
}
