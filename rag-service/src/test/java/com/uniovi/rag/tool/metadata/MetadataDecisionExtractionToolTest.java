...
class MetadataDecisionExtractionToolTest {

    private ChatClient chatClient;
    private ContextRetriever retriever;
    private DocumentContentExtractor extractor;
    private MetadataDecisionExtractionTool tool;
    ...
    void execute_emptyRetrieval_returnsToolResult() {
        ToolResult result = tool.execute(ToolExecutionContext.of("decisiones", QueryType.DECISION_EXTRACTION, null));
        assertNotNull(result);
        assertNotNull(result.result());
        assertEquals("MetadataDecisionExtractionTool", result.source());
    }
}
