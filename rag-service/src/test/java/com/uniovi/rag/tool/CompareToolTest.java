...
class CompareToolTest {
    ...
    private CompareTool tool;

    @BeforeEach
    void setUp() {
        chatClient = ChatClientTestSupport.clientWithUserPromptReturning("No data to compare.");
        ...
        tool = new CompareTool(chatClient, retriever, extractor);
    }

    @Test
    void execute_emptyRetrieval_returnsToolResult() {
        ToolResult result = tool.execute(ToolExecutionContext.of("comparar asistentes", QueryType.COMPARE, null));
        assertNotNull(result);
        assertNotNull(result.result());
        assertEquals("CompareTool", result.source());
    }
}
