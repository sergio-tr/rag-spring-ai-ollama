...
class ChatRuntimeUsesConversationRetrievalOverrideTest {

    @Test
    void mergedConversationConfig_runtimeOverrideBeatsPresetRetrieval() throws Exception {
        ...
        when(preset.getValues()).thenReturn(Map.of("topK", 5, "similarityThreshold", 0.9));
        ...
        when(conv.getRuntimeOverride()).thenReturn(Map.of("topK", 12, "similarityThreshold", 0.4));
        ...
        assertEquals(12, terminal.get("topK").asInt());
        assertEquals(0.4, terminal.get("similarityThreshold").asDouble());
    }
