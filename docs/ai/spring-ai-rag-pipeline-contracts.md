...
| Legacy string synthesis | `AnswerGenerationKernel` (via `ResponseSynthesisPipeline`) | Retriever, optional advisor, `ChatRequestSpecFactory` | Draft answer string — **not** the primary orchestrated path |
...
3. **`QuestionAnswerAdvisor`** is composed at **call time** on the chat path that uses it (see `RagConfiguration` / `ProcessQueryService` integration), not on the base `ChatClient` bean, to avoid circular dependencies.
