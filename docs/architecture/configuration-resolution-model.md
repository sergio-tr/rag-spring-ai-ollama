...
4. **Output:** materialize `ResolvedRuntimeConfig`; optional **persisted** row in `resolved_config_snapshot` via `ResolvedConfigSnapshotApplicationService` (insert-only, `config_hash`, §6.1 in [DATA_MODEL.md](DATA_MODEL.md)). Knowledge execute-without-pin persists the same row shape plus **`knowledgeBuildProjection`** under `payload_jsonb` and a `config_hash` that includes that nested map ([DATA_MODEL.md](DATA_MODEL.md) §6.1).
